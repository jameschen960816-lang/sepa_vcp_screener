...
MIN_PRICE         = 8.0         # 最低股價（美元）
MIN_AVG_VOLUME    = 500_000     # 最低日均成交量（股）
...
def run() -> None:
    ...
    for t, df in full_data.items():
        ...
        gainers[t]["avg_volume"] = round(avg_volume / 10000, 1)
        sepa_pass.append(t)

    print(f"通過篩選（股價≥${MIN_PRICE}、日均量≥{MIN_AVG_VOLUME//10000}萬、SEPA {SEPA_MIN_PASS}/7、RS≥{MIN_RS}、距高點≤{MAX_FROM_HIGH_PCT}%）：{len(sepa_pass)} 支")
    if not sepa_pass:
        print("無股票同時通過漲幅 + 基本篩選 + SEPA 條件。")
        return
    ...
    for t in sepa_pass:
        ...
        if fundamentals["pass"]:
            ...
            print(
                f"  ✓ {t}  漲幅={gainers[t]['gain_pct']:.1f}%  "
                f"日均量={gainers[t]['avg_volume']:.1f}萬  "
                f"距高點={gainers[t]['from_high_pct']:.1f}%  "
                f"季EPS年增={fundamentals['eps_growth']}%  "
                f"年營收增={fundamentals['rev_growth']}%  "
                f"利潤率={margin_str}  ROE={fundamentals['roe']}%"
            )
        else:
    ...
    if not final:
        print("今日無股票同時通過漲幅 + 基本篩選 + SEPA + 基本面條件。")
